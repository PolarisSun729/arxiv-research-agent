...
rem It remembers the last successfully counted full day in a separate local state file.
...
set "STATE_FILE=%STATE_DIR%\sync_arxiv_oai_since_last_run_count_only.state"
set "META_FILE=%STATE_DIR%\sync_arxiv_oai_since_last_run_count_only.meta.json"
...
"%PYTHON_EXE%" "%PROJECT_ROOT%sync_arxiv_oai.py" --from "!FROM_DATE!" --until "!UNTIL_DATE!" --count-only --meta-file "%META_FILE%" --interval %INTERVAL_SECONDS% --timeout %TIMEOUT_SECONDS% --max-retries %MAX_RETRIES% --log-level %LOG_LEVEL%
