...
class ArxivSearchService:
    ...
    def build_submitted_date_query(self, days_ago: Optional[int] = 30) -> str:
        ...
        start_date = end_date - timedelta(days=days_ago) if days_ago else end_date - timedelta(days=30)
    ...
    def search_papers(self, 
        ...
        """
        try:
            if submitted_days_ago and submitted_days_ago > 0:
            ...
            logger.info(f"Searching arXiv with query: '{search_query}', id_list: {id_list}, max_results: {max_results}")
            
            url = self.build_query_url(search_query, id_list, max_results, start, sort_by, sort_order)
    ...
    def search_advanced(self, 
        ...
        if title:
            queries.append(self.build_field_query(SearchField.TITLE, title))
        if author:
            queries.append(self.build_field_query(SearchField.AUTHOR, author))
        if abstract:
            queries.append(self.build_field_query(SearchField.ABSTRACT, abstract))
        if category:
            queries.append(self.build_field_query(SearchField.SUBJECT_CATEGORY, category))
        if comment:
            queries.append(self.build_field_query(SearchField.COMMENT, comment))
        if journal_ref:
            queries.append(self.build_field_query(SearchField.JOURNAL_REFERENCE, journal_ref))
        if report_number:
            queries.append(self.build_field_query(SearchField.REPORT_NUMBER, report_number))
        
        if not queries and not id_list:
            raise ValueError("At least one search condition must be provided")
        
        combined_query = self.combine_queries(queries, operator) if queries else ""
