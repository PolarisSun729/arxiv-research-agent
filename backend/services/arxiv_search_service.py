...
def _is_non_empty_text(value: Optional[str]) -> bool:
...
def _quote_arxiv_text(value: str) -> str:
...
def _build_arxiv_field_clause(prefix: str, value: str) -> str:
    normalized = _normalize_text_value(value)
    if not normalized:
        return ""
    if prefix == "cat":
        return f"{prefix}:{normalized}"
    return f"{prefix}:{_quote_arxiv_text(normalized)}"


def _combine_arxiv_clauses(clauses: List[str], operator: str = "AND") -> str:
...
def _normalize_id_list(id_list: Optional[List[str]]) -> List[str]:
    if not id_list:
        return []
    return [str(item).strip() for item in id_list if str(item).strip()]


def _validate_arxiv_search_request(
    *,
...
def build_arxiv_query_from_structured_params(
    *,
    ...
    normalized_title = _normalize_text_value(title_query)
    normalized_author = _normalize_text_value(author_query)
    normalized_abstract = _normalize_text_value(abstract_query)
    normalized_comment = _normalize_text_value(comment_query)
    normalized_journal_ref = _normalize_text_value(journal_ref_query)
    normalized_report_number = _normalize_text_value(report_number_query)

    clauses: List[str] = []
    if normalized_query:
        clauses.append(_build_arxiv_field_clause("all", normalized_query))
    if normalized_title:
        clauses.append(_build_arxiv_field_clause("ti", normalized_title))
    if normalized_author:
        clauses.append(_build_arxiv_field_clause("au", normalized_author))
    if normalized_abstract:
        clauses.append(_build_arxiv_field_clause("abs", normalized_abstract))
    if normalized_categories:
        category_clauses = [f"cat:{category}" for category in normalized_categories]
        category_clause = _combine_arxiv_clauses(category_clauses, normalized_category_operator)
        if len(category_clauses) > 1:
            category_clause = f"({category_clause})"
        clauses.append(category_clause)
    if normalized_comment:
        clauses.append(_build_arxiv_field_clause("co", normalized_comment))
    if normalized_journal_ref:
        clauses.append(_build_arxiv_field_clause("jr", normalized_journal_ref))
    if normalized_report_number:
        clauses.append(_build_arxiv_field_clause("rn", normalized_report_number))
    ...
    final_search_query = _combine_arxiv_clauses(clauses, normalized_field_operator)
...
class ArxivSearchService:
    ...
    def build_submitted_date_query(self, days_ago: Optional[int] = 30) -> str:
        ...
        """
        end_date = datetime.now(timezone.utc)
        start_date = end_date - timedelta(days=days_ago) if days_ago is not None else end_date - timedelta(days=30)
    ...
    def search_papers(self, 
        ...
        try:
            normalized_search_query = _normalize_text_value(search_query)
            if submitted_days_ago is not None and submitted_days_ago >= 0:
                date_query = self.build_submitted_date_query(submitted_days_ago)
                if normalized_search_query:
                    normalized_search_query = f"({normalized_search_query}) AND {date_query}"
                else:
                    normalized_search_query = date_query
            return self.search(
